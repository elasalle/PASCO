...
if __name__ == '__main__':
    ...
    for i, graph_type in enumerate(graph_types):
        print(graph_type)
        fig, axs = plt.subplots(1,nb_max_graphs, figsize=(5*nb_max_graphs, 5), sharey="row")
        for jax, (ax, graph) in enumerate(zip(axs, errors[graph_type].keys())):
            ...
            if graph_type=="SBM":
                ...
                ax.set_title(graph)
        legend_height = 0.07
        plt.tight_layout(pad=0.1, w_pad=1,  h_pad=1.5, rect=(0,legend_height*2,1,1))
        ...
        fig.savefig(plot_dir+"/"+fig_name+"_"+graph_type+".pdf")



    ##################################
    # all in one plot with two lines #
    ##################################


    # nb_graph_types = len(graph_types)
    # nb_max_graphs = max([len(errors[graph_type]) for graph_type in graph_types])
    # fig, axss = plt.subplots(nb_graph_types,nb_max_graphs, figsize=(5*nb_max_graphs, 5*nb_graph_types), sharey="row", squeeze=False)
    ...
    #             ax.set_title(graph)
    # legend_height = 0.03
    # plt.tight_layout(pad=1.5, h_pad=1.5, rect=(0,legend_height*2,1,1))
    # plt.legend(loc='center', bbox_to_anchor=(0.5, legend_height), bbox_transform=fig.transFigure, shadow=True, ncol=len(coarsening_methods))
