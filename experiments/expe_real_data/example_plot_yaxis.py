...
if __name__ == '__main__':
    ...
    all_solvers = ['SC','CSC','louvain','leiden','MDL','infomap']
    sorted_solvers = all_solvers.copy()
    sorted_solvers.sort()
    results = {}
    ...
    fig, axss = plt.subplots(
        len(solvers), len(perfs), 
        figsize=(7, 5.5),
        sharey=True, sharex="col", squeeze=False)
    plt.subplots_adjust(top= 0.98, bottom=bottom, left=0.2, right=0.95, 
                        hspace=0.15, wspace=0.2)
    for i, (axs,solver) in enumerate(zip(axss,solvers)):
    ...
    if save:
        plt.savefig(fig_dir + "example_" + display_time + "_time_vs_" + display_perf + "_perf_" + '_'.join(datasets) + "_" + suffix +".pdf")
