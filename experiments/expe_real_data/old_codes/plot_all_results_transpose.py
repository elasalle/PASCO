...
if __name__ == '__main__':
    ...
    fig, axss = plt.subplots(
        len(perfs), len(solvers_to_plot), 
        figsize=(18, 3*len(perfs)),
        sharex=True, sharey="row")
    plt.subplots_adjust(top= 0.95, bottom=bottom, left=0.07, right=0.98, 
                        hspace=0.2, wspace=0.2)
    for i, (axs,perf) in enumerate(zip(axss,perfs)):
        for jax, (ax, solver) in enumerate(zip(axs, solvers_to_plot)):
            for ds in datasets:
            ...
            if jax==0:
                if relative:
                    if perf=="ami":
                        ax.set_ylabel(perfs_name[perf] + r" $\rightarrow$")
                    else:
                        ax.set_ylabel(r" $\leftarrow$" + "RE " + perfs_name[perf])
                else:
                    ax.set_title(perf)
    ...
    for i in range(len(perfs)):
        axss[i][0].yaxis.set_label_coords(-0.25, 0.5)

    if save:
        plt.savefig(fig_dir + "perf_vs_"+ "time_transpose_" + '_'.join(solvers)+ "_" + suffix +".pdf")
    # for axs in axss:
    #     for ax in axs:
