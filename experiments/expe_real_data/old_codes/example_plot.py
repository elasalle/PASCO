...
if __name__ == '__main__':
    ...
    sorted_solvers.sort()
    saving_file_name = res_dir + '/res_arxiv_CSC_MDL_SC_infomap_leiden_louvain_ot.pickle' # when using pickle to save the results
    ...
    fig = plt.figure(figsize=(4.5*len(perfs), 5*len(rhos)), num='time')

    # Define the grid for the plots only (without colorbars)
    gs = gridspec.GridSpec(len(rhos), len(perfs), 
                        hspace=0.3,  # Keep spacing between actual plots
                        top=0.87, bottom=0.27, left=0.2, right=0.75)  # Right boundary at 0.85 to leave space for colorbars
    ...
    for jax, (ax, perf) in enumerate(zip(axs, perfs)):
        ...
        ax.grid(alpha=0.5)
        ax.set_xlim(1.1,1200)
        ax.set_ylim(0.11,0.44)
        ax.set_xscale("log")
        ax.set_xlabel("time")
    ...
    colorbar_mostleft = rightmost_plot_right + 0.15
    ...
    if save:
        fig.savefig(fig_dir + "example.pdf")
